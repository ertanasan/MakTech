// Created by OverGenerator
/*Section="Imports"*/
import { ModelBase } from '@otmodel/model-base';
import { RelationId } from '@otmodel/relation-id.model';

/*Section="ClassHeader"*/
export class WorkingHours extends ModelBase {
    public WorkingHoursId = 0;
    public Event = 1060;
    public Organization = 1;
    public Deleted = false;
    public CreateDate = new Date();
    public UpdateDate?: Date;
    public CreateUser = 0;
    public UpdateUser?: number;
    public CreateChannel = 0;
    public CreateBranch = 0;
    public CreateScreen = 0;
    public StoreCode?: string;
    public StoreName?: string;
    public OpeningTime?: Date;
    public ClosingTime?: Date;
    public OpenUserName?: string;
    public CloseUserName?: string;
    public Store?: number;
    public OpenUser?: number;
    public CloseUser?: number;
    public Note?: string;

    constructor() {
        super();
    }

    /*Section="Method-setId"*/
    setId(id: number) {
        this.WorkingHoursId = id;
    }

    /*Section="Method-getId"*/
    getId(): number | RelationId {
        return this.WorkingHoursId;
    }

    /*Section="CustomCodeRegion"*/
    //#region Customized
    // Keep your custom code in this region.
    //#endregion Customized

    /*Section="ClassFooter"*/
}
