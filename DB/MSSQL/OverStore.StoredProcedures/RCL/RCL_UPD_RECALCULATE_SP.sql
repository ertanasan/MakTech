CREATE PROCEDURE RCL_UPD_RECALCULATE_SP @StoreId INT AS  
BEGIN  
	...
	DECLARE @expensereturn NUMERIC(22,6), @recovered NUMERIC(22,6), @totalopenexpense NUMERIC(22,6)    
	...
	SELECT row_number() over (order by RECONCILIATION_DT) rowno, STORERECID, ZET_AMT, DEFINEDADVANCE_AMT, CASH_AMT, CARD_AMT, EXPENSE_AMT, EXPENSERETURN_AMT, RECOVERED_AMT, ADJUSTMENT_AMT
		 , SUM(EXPENSE_AMT - EXPENSERETURN_AMT) OVER (PARTITION BY STORE ORDER BY RECONCILIATION_DT ROWS UNBOUNDED PRECEDING) TOTALOPENEXPENSE_AMT 
	  FROM RCL_STORE
	 WHERE STORE = @StoreId
	   AND COMPLETE_FL = 'Y'
	   AND CASH_AMT + CARD_AMT > 0
	 ORDER BY RECONCILIATION_DT
	 
 
	SET NOCOUNT ON  
	OPEN rcl_cursor    
	FETCH NEXT FROM rcl_cursor INTO @rowno, @storeRecId, @zet, @definedAdvance, @cash, @card, @expense, @expensereturn, @recovered, @adjustment, @totalopenexpense
	...
	BEGIN    
	  ...
	  IF @definedAdvance - @advance - @totalopenexpense > 5 
