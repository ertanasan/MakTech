CREATE PROCEDURE [dbo].[WHS_INS_WAYBILLBYTRANSFERPRODUCTDETAIL_SP] @TransferProductDetailId INT AS
BEGIN
	...
	DECLARE @TransferDate DATE, @MasterId INT, @MaxRowNo INT, @control VARCHAR(100);

	SELECT @control =sth_Guid, @TransferDate = sth_create_date, @MasterId = TD.TRANSFERPRODUCT
	...
	IF @control != null
	BEGIN
		;THROW 100001, 'Mikroda bu ürüne ait transfer kaydı mevcut. Inset işlemi başarısız.', 1;
		RETURN;
	END

	IF DATEDIFF(day, @TransferDate, GETDATE()) > 10
	BEGIN
		;THROW 100001, 'Transfer üstünden 10 gün geçmiş. Mikroya URUNTRANSFER insert işlemi başarısız.', 1;
		RETURN;
	END

	ELSE -- IF @control = 0
	
	SELECT @MaxRowNo = Max(sth_satirno)
	   FROM MIK_TRANSACTION20_SYN
	  WHERE sth_evraktip = 2
        AND sth_evrakno_seri = 'OMT'
        AND sth_evrakno_sira = @MasterId
    ...
    SELECT TD.TRANSFERPRODUCTDETAILID, ROW_NUMBER() OVER (ORDER BY TD.TRANSFERPRODUCTDETAILID) ROWNO,
     ...
     WHERE TP.TRANSFERPRODUCTID = @MasterId 
	   AND TD.TRANSFERPRODUCTDETAILID = @TransferProductDetailId
    ...
    SELECT NEWID() sth_Guid, 0 sth_DBCno, 0 sth_SpecRECno, 0 sth_iptal, 16 sth_fileid, 0 sth_hidden, 0 sth_kilitli, 0 sth_degisti, 0 sth_checksum, 10 sth_create_user
         ...
         , @TransferDate sth_tarih, 2 sth_tip, 6 sth_cins, 0 sth_normal_iade
         , 2 sth_evraktip, 'OMT' sth_evrakno_seri, @MasterId sth_evrakno_sira, CASE WHEN @MaxRowNo IS NULL THEN 1 ELSE (@MaxRowNo + 1) END sth_satirno
         , cast(@MasterId AS VARCHAR(20)) sth_belge_no, @TransferDate sth_belge_tarih, PRODUCTCODE_NM sth_stok_kod
         ...
         , @TransferDate sth_malkbl_sevk_tarihi, '' sth_cari_srm_merkezi, '' sth_stok_srm_merkezi, @emptydate sth_fis_tarihi         
