CREATE PROCEDURE SLS_RPT_DAILYTURNOVER_SP @Date DATE = NULL AS 
BEGIN
	...
	WITH CARDAMT AS (	
	SELECT STORE, CASH_AMT, CARD_AMT, BANK_AMT
	  FROM RCL_STORE
	...
	SELECT ROW_NUMBER() OVER (ORDER BY SELECTEDDAILYTOTAL DESC) ROWNO
		 , STOREID, STORE_NM, TRANSACTION_DT, SELECTEDDAILYTOTAL, SELECTEDDAILYTOTAL - ISNULL(CARD_AMT,0) CASH_AMT
		 , CARD_AMT, CARD_AMT ENDOFDAY_AMT, SLPTOTAL_AMT, REFUND_AMT, RECEIPT_CNT
		 , SELECTEDDAILYTOTAL - ISNULL(CARD_AMT,0) + ISNULL(SLPTOTAL_AMT,0) LOOMIS
		 , BANK_AMT, SELECTEDDAILYTOTAL + ISNULL(SLPTOTAL_AMT,0) TURNOVER
		 , ZBALANCE, ZTOTAL, ZTOTALWOSLP, DAILYTOTAL, DAILYTOTALWOSLP
	  FROM (
	SELECT STOREID, STORE_NM, @DATE TRANSACTION_DT
		 ...
		 , ZTOTAL, ZTOTALWOSLP, DAILYTOTAL, DAILYTOTALWOSLP
