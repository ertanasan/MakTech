CREATE PROCEDURE SLS_LST_MIKROTRANSFERDATA_SP  @StartDate DATE, @EndDate DATE, @CheckDate INT AS    
BEGIN    
	...
	STORES AS 
	(SELECT STOREID, STORE_NM, MAX(CRB.BRAND_NM) TERMINAL
	   FROM STR_STORE ST
	   JOIN STR_CASHREGISTER CR ON ST.STOREID = CR.STORE
	   JOIN STR_CASHREGISTERMODEL CRM ON CR.CASHREGISTERMODEL = CRM.CASHREGISTERMODELID
	   JOIN STR_CASHREGISTERBRAND CRB ON CRM.BRAND = CRB.CASHREGISTERBRANDID
	  GROUP BY STOREID, STORE_NM) 
	...
	SELECT ST.STOREID, ST.STORE_NM, ST.TERMINAL, CAST(C.TRANSACTION_DT AS date) TRANSACTION_DT  
	  ...
	  JOIN STORES ST ON C.STOREID = ST.STOREID  
