CREATE PROCEDURE MIK_INS_POS_SP @Day DATE, @PosIdList VARCHAR(MAX) AS
BEGIN
	-- RETURN;
	...
	SELECT @TranNo = ISNULL(MAX(cha_evrakno_sira),0)+1 FROM MIK_CURRENTTRANSACTION20_SYN WHERE cha_evrakno_seri = 'MPOS'
	SELECT @FisSiraNo = ISNULL(MAX(fis_sira_no), 0)+1 FROM MIK_ACCOUNTING20_SYN (NOLOCK)  WHERE fis_tarih = @Day;  -- fis_firmano = 0 AND fis_maliyil = YEAR(@today); -- AND 
	SELECT @FisYevmiyeNo = ISNULL(MAX(fis_yevmiye_no), 0)+1 FROM MIK_ACCOUNTING20_SYN (NOLOCK)  WHERE fis_firmano = 0 AND fis_maliyil = YEAR(@Day); -- AND fis_tarih = @today;  
	...
	SELECT * INTO #list FROM [dbo].[SYS_CSVTOTABLE_SP] (@PosIdList,',')

	SELECT *, ROW_NUMBER() OVER (ORDER BY LEFT(CHAKOD,3) DESC, POSREF_TXT) ROWNO
	  INTO #transactions
	  FROM (
	SELECT NEWID() GUID1, T.BANKPOSTRANID, T.POSREF_TXT, T.VALUE_DT, T.QUANTITY_QTY, T.DEBIT_AMT, P.STORE, ST.STORE_NM, P.BANK
		 , 1 CHATIP, 4 CHACARICINS, 0 CHAGRUPNO, ISNULL(PROP.VALUE_TXT, '100.01.'+RIGHT('000'+ISNULL(CAST(P.STORE AS VARCHAR(10)),''),3)) CHAKOD
	  FROM ACC_BANKPOSTRAN T
	  JOIN STR_POS P ON T.POSREF_TXT = P.POSCODE_TXT
	  JOIN STR_STORE ST ON P.STORE = ST.STOREID
	  LEFT JOIN STR_PROPERTY PROP ON PROP.PROPERTYTYPE = 10 AND PROP.STORE = ST.STOREID
	 WHERE BLOCKED_DT = @Day
	   AND (LEN(ISNULL(@PosIdList,'')) = 0 OR T.BANKPOSTRANID IN (SELECT String FROM #list))
	   AND T.MIKROSTATUS_CD = 0
	 UNION ALL
	SELECT NEWID() GUID1, T.BANKPOSTRANID, T.POSREF_TXT, T.VALUE_DT, T.QUANTITY_QTY, T.DEBIT_AMT, P.STORE, ST.STORE_NM, P.BANK
	     , 0 CHATIP, 2 CHACARICINS, 7 CHAGRUPNO
		 , ISNULL(PROP.VALUE_TXT, '102.07.'+RIGHT('000'+ISNULL(CAST(P.STORE AS VARCHAR(10)),''),3)) CHAKOD
	  FROM ACC_BANKPOSTRAN T
	  JOIN STR_POS P ON T.POSREF_TXT = P.POSCODE_TXT
	  JOIN STR_STORE ST ON P.STORE = ST.STOREID
	  LEFT JOIN STR_PROPERTY PROP ON PROP.PROPERTYTYPE = 13 AND PROP.STORE = ST.STOREID
	 WHERE BLOCKED_DT = @Day
	   AND (LEN(ISNULL(@PosIdList,'')) = 0 OR T.BANKPOSTRANID IN (SELECT String FROM #list))
	   AND T.MIKROSTATUS_CD = 0
	 ) A
	...
	SELECT *, ROW_NUMBER() OVER (ORDER BY LEFT(CHAKOD,3) DESC, STORE) ROWNO
	  ...
	  FROM (
	SELECT *, 
		   CASE WHEN LEFT(CHAKOD,3) = 100 THEN '100.02.001' ELSE CASE WHEN BANK = 1 THEN '127.02.003' ELSE '127.02.004' END END ACCCODE,
		   CASE WHEN LEFT(CHAKOD,3) = 100 THEN -1*STORETOTAL ELSE STORETOTAL END AMOUNT,
		   CONVERT(NVARCHAR(120), 'Gn.Vir.Dek. : MPOS-'+CAST(@TranNo AS VARCHAR(20))+'/'+CONVERT(VARCHAR(30), @Day, 104)+'/KREDİ KARTI SATIŞLARI/'+CHAKOD+'/'+STORE_NM, 120) DESC1
	  FROM (
	SELECT CHAKOD, STORE, STORE_NM, BANK, SUM(DEBIT_AMT) STORETOTAL, MIN(GUID1) TRANGUID
	  FROM #transactions
	 GROUP BY CHAKOD, STORE, STORE_NM, BANK) A ) B
	...
	SELECT GUID1, 0, GETDATE(), 'MPOS', @TranNo, ROWNO, @Day, @Day, CHAKOD, ' ' cha_kasa_hizkod, 
	 ' ' cha_karsisrmrkkodu, DEBIT_AMT, 0 cha_aratoplam, 0 cha_vergipntr, 0, 0, 0, 0, 
	 CONVERT(NVARCHAR(40), LEFT('KREDİ KARTI : ' + POSREF_TXT + ' - ' + CONVERT(varchar(20), VALUE_DT, 104), 40)), @Day,
	...
	SELECT NEWID(), 0, 0, 0, 2, 0, 0, 0, 0, 10, GETDATE(), 10, GETDATE(), 0, 0, YEAR(@Day), @Day, @FisSiraNo, 0 fis_tur, ACCCODE, ROWNO, DESC1, 
		   AMOUNT fis_meblag0, AMOUNT / 5.7997 fis_meblag1, AMOUNT fis_meblag2, 0, 0, 0, 0, 
		   ' 'fis_sorumluluk_kodu, 2, TRANGUID, 0, @evraktip, 'MPOS', @TranNo, @Day fis_tic_belgetarihi, @FisYevmiyeNo, 0, 0, 0, 0, ' '
	  from #accrows
	
	DELETE FROM MIK_ACCOUNTINGDETAIL20_SYN WHERE mfd_evrak_seri = 'MPOS' and mfd_evrak_sira = @TranNo
	...
	SELECT NEWID(), 0, 0, 0, 243, 0, 0, 0 mfd_degisti, 0, 10, GETDATE(), 10, GETDATE(), 2 mfd_ticari_tip	
		 , @evraktip mfd_evraktip, 'MPOS', @TranNo, 0 mfd_bsbakonututar, 0, @Day, 0 mfd_tutarnereden, 0, CONVERT(NVARCHAR(20), LEFT('KREDİ KARTI SATIŞLARI',20)) mfd_belgeno
		 , 0, 0, 0, 0, N'KREDİ KARTI SATIŞLARI', 0 mfd_caritutar, 5 mfd_kisaevraktipi, 0, 0, 0, N'Genel Amaçlı Virman Dekontu', 0
	
	UPDATE B 
	   SET B.MIKROTRANCODE_TXT = 'MPOS - '+CAST(@TranNo AS VARCHAR(20)),
		   B.MIKRO_TM = GETDATE(),
		   B.UPDATEUSER = ISNULL(dbo.SYS_GETCURRENTUSER_FN(),1),
		   B.UPDATE_DT = GETDATE(),
		   B.MIKROSTATUS_CD = 1
	  FROM ACC_BANKPOSTRAN B
	  JOIN #transactions X ON B.BANKPOSTRANID = X.BANKPOSTRANID
